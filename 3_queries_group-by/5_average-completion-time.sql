-- Objective: Get currently enrolled students' average assignment completion time.
...
SELECT students.name as student, SUM(assignments.duration) / count(assignments.*) as average_assignment_duration
FROM assignments
JOIN assignment_submissions ON assignments.id = assignment_id
JOIN students ON students.id = student_id
WHERE end_date IS NULL
GROUP BY average_assignment_duration
